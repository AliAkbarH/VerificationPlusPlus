...
void
SpecCheckVocab::getVocabPartialBadAssignments() 
{
   bool repeat = false;
do {    
...
void
SpecCheckVocab:: printDeclarations(){
/*
...
void 
SpecCheckVocab::assertParsedFormulae() {
    unsigned int i = 0;
 ...
 */
void
SpecCheckVocab::choiceAssertRetract(unsigned int offset){
  /*
...
void
SpecCheckVocab::computeEliminatedPatternsFromBadPartialAssignments( Z3_model model, vector<string> & partials) {
...
void
SpecCheckVocab::printUnsatCore( unsigned int core_size, Z3_ast * core) 
{
...
void
SpecCheckVocab::computeEliminatedPatternsFromUnsatChoice (
        unsigned int core_size, Z3_ast * core) {
    // if result is not 1:if it is unsat, 
    // store the unsat_cores so as not to repeat them
    //store unsat cores into the vector unsat_cores
...
//Maya
void
SpecCheckVocab::displayModel( Z3_model &model, vector<string>& Display) {
  Name2VarMap_t :: iterator it = name2var.begin();
...
bool 
SpecCheckVocab :: writeEspressoFile( const char * filename, bool bPrintUnsatCore, bool bPrintNoChoices) {
  ofstream oEspresso;
...
void
SpecCheckVocab:: abcSimplify(const string & filename,
    vector < vector<vocab_value_t> > & kissChoices ){
...
void
SpecCheckVocab:: espressoSimplify(){
