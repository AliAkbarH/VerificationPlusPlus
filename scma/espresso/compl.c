...
pcover compl_merge(T1, L, R, cl, cr, var, lifting)
 ...
 */
void compl_lift_onset(A1, T, bcube, var)
pcube *A1;
pcover T;
pcube bcube;
int var;
 ...
 */
void compl_lift_onset_complex(A1, T, var)
...

bool simp_comp_special_cases(T, Tnew, Tbar)
pcube *T;			/* will be disposed if answer is determined */
...

bool simplify_special_cases(T, Tnew)
pcube *T;			/* will be disposed if answer is determined */
pcover *Tnew;			/* returned only if answer determined */
{
    register pcube *T1, p, ceil, cof=T[0];
